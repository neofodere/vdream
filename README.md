...
`vdream.exe:` The main executable of the emulator. Launches the UI and manages user interaction.<br>
`vdream_core.exe:` The low-level emulation core. Handles CPU, memory, graphics, audio and hardware logic.<br>
`vdream.dll:` Native library used by the emulator to interface with system-level functions and provide performance‑critical routines.<br>
`dream.config:` Configuration file for user preferences such as language, UI settings and general emulator options.<br>
`vdream_core.config:` Configuration file for internal emulation parameters such as timing, video mode, audio behavior and hardware accuracy.<br>
`vdream.deps.json:` Dependency manifest generated by .NET. Lists all required libraries and runtime components for the application.<br>
