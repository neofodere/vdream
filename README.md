...
```
\saves: Folder with your savestates.

vdream.exe: The main executable of the emulator. Launches the UI and manages user interaction.

vdream_core.exe: The low-level emulation core. Handles CPU, memory, graphics, audio and hardware logic.

vdream.dll: Native library used by the emulator to interface with system-level functions and provide performance‑critical routines.

vdream.config: Configuration file for user preferences such as language, UI settings and general emulator options.

vdream_core.config: Configuration file for internal emulation parameters such as timing, video mode, audio behavior and hardware accuracy.

vdream.deps.json: Dependency manifest generated by .NET. Lists all required libraries and runtime components for the application.

vdream.runtimeconfig.json: Runtime configuration file generated by .NET. Defines the .NET runtime version and environment settings required to run the emulator.
```
